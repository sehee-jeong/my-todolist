...
**작업 내용:**
- [x] SC-H-01 ~ SC-H-11 (정상 흐름 11건) 자동화 테스트로 검증
- [x] SC-E-01 ~ SC-E-12 (예외 흐름 12건) 자동화 테스트로 검증
- [x] SC-E2E-01: 직장인 김민준 시나리오 전체 흐름 자동화 테스트로 검증
- [x] SC-E2E-02: 대학생 이수아 시나리오 전체 흐름 자동화 테스트로 검증

**완료 조건:**
- [x] 25개 시나리오 전체 기대 결과 일치 (32개 테스트 통과, 커버리지 93.98%)
- [x] NFR-S-01 ~ NFR-S-04 보안 항목 확인
  - [x] DB에 저장된 비밀번호가 bcrypt 해시값인지 확인 (auth.service.ts: bcrypt.hash cost 10)
  - [x] JWT 만료 24시간 확인 (auth.service.ts: expiresIn '24h', SC-H-02 테스트)
  - [x] 타인 토큰으로 접근 시 403 반환 확인 (SC-E-07, SC-E-09, SC-E2E-02 테스트)
...
**작업 내용:**
- [x] API 평균 응답 시간 500ms 이하 확인 (NFR-P-01) — 로컬 실측: login 76ms, GET /todos 3ms, health 1ms
- [x] 할 일 목록 조회 1초 이하 확인 (NFR-P-02) — 로컬 실측: 평균 3.6ms (10회)
- [x] `.env` 파일 Git 미포함 확인 (`.gitignore` 검토)
- [x] 에러 응답에 스택 트레이스 미포함 확인
- [ ] Vercel 배포 설정 확인 — PRD 명시: 배포 환경 구성은 MVP 범위 외

**완료 조건:**
- [x] 주요 API 응답 500ms 이하 (로컬 실측: login 76ms, GET /todos 3ms — PASS)
- [x] `.env` 가 `.gitignore` 에 포함 (backend/.gitignore, root .gitignore 모두 확인)
- [x] 프로덕션 에러 응답에 스택 트레이스 미노출 (app.ts errorHandler: 500→'Internal Server Error' 고정)
- [ ] MVP 배포 완료 URL 접속 확인
