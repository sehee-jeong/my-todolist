...
GitLab CI Pipeline
    ↓
공용 Runner (shared-kr-runner, HCloud)
    ↓
Docker 이미지 (빌드/배포 스크립트 실행)
    ↓
SSH로 PWC에 배포 (10.90.170.168)
```
...
- 보안/권한 정책(Defender, 백신, 로컬 관리자 권한 제한 등)으로
  **OpenSSH 설치·활성화** 시도 자체가 어려움
...
| 구분 | 상태 | 설명 |
|------|------|------|
| **네트워크 (길)** | ✅ 정상 | SRMS 방화벽 승인 완료 |
| **엔드포인트 (문)** | ❌ 차단 | PWC 정책/권한 제약으로 SSH 서비스 구동 불가 |
...
- 설정 문제보다 **정책/권한/보안 제약**이 더 큰 허들
- 다음에는 **정식 서버/프로젝트 선행이 필수**라고 느꼈습니다
- 다 막아놨으 ㅡㅅㅡ
